...
function checkSeatbeltSwitch()
	...
	local aircraft = {
		...
		["A20N"] = function()	-- for Toliss A320 NEO (A20N)
			dataref("tolissA20NSB", "AirbusFBW/SeatBeltSignsOn", "readonly")
			if tolissA20NSB == 1 then
				SeatbeltMonitor = 1
			else
				SeatbeltMonitor = 0
			end
		...
		["A339"] = function()	-- for Toliss A330
			dataref("tolissA339SB", "AirbusFBW/SeatBeltSignsOn", "readonly")
			if tolissA339SB == 1 then
				SeatbeltMonitor = 1
			else
				SeatbeltMonitor = 0
			end
